...
contract DeployTechtreeGraphRegistryV1BaseSepolia is Script {
    error WrongChain(uint256 actualChainId);
    error RolesNotDistinct(address deployer, address owner, address registrar);

    /// @notice Prepares a Base Sepolia deployment from environment-provided role addresses and key.
    ...
    function run() external returns (TechtreeGraphRegistryV1 deployed) {
        if (block.chainid != 84_532) revert WrongChain(block.chainid);
