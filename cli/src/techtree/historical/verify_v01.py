"""Checking a v0.1 local proof, offline, from exactly the bytes it stored.

This is the v0.1 release's verifier, moved into the historical boundary
unchanged when the live proof moved to the v0.2 documents. It reads the frozen
v0.1 Campaign, manifests, receipts and report, and it is the only verifier a
v0.1 proof is ever read through: a v0.1 bundle keeps verifying with the same
outcome it had, and nothing on the live write path reaches this module.
...
from techtree.historical.receipts_v01 import (
...
from techtree.receipts.set import RECEIPT_SET_INVALID, ReceiptSetManifest
