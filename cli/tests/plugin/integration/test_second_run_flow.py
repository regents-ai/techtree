...
def test_the_phone_review_carries_what_an_approval_turns_on(
    journey: PluginServices,
) -> None:
    """The worst case still carries every fact the approval is made from.

    Measured on the longest revision the suite produces, because that is the
    case where the review has the most to say and the least room to say it.
    What a phone gets is decided by what a reader can act on - a digest cannot
    be checked by eye and the plugin's own bookkeeping is not a decision - and
    never by a size that some guess made unaffordable.
    """
    _call(journey, "techtree_run_status", run_id=FIRST_RUN)
    journey = _revising(journey, LONG_V2)
